...
var DeviceOrientationControls = function () {
  ...
  createClass(DeviceOrientationControls, [{
    ...
    value: function deviceOrientationHandler(event) {
      this.deviceOrientation.alpha = THREE.Math.degToRad(event.alpha);
      this.deviceOrientation.beta = THREE.Math.degToRad(event.beta);
      this.deviceOrientation.gamma = THREE.Math.degToRad(event.gamma);
    }
  }, {
    key: 'resetRotation',
    ...
    value: function update() {
      ...
      var orientationRotation = new THREE.Quaternion();
...
var AppControls = function () {
  ...
  createClass(AppControls, [{
    key: 'setVRDisplay',
    value: function setVRDisplay(display) {
      this.vrControls = new VRControls(this._camera, display);
    }
  }, {
    key: 'setCamera',
    value: function setCamera(camera) {
      this._camera = camera;
      if (this.nonVRControls) {
    ...
    value: function frame(frameOptions) {
      if (this.vrControls) {
        var display = this.vrControls.getVRDisplay();
  ...
  createClass(VREffect, [{
    ...
    value: function requestPresent() {
      ...
      return this.vrDisplay.requestPresent([{
        ...
        var leftParams = _this.vrDisplay.getEyeParameters('left');
        var rightParams = _this.vrDisplay.getEyeParameters('right');

        _this.renderer.setPixelRatio(1);
        _this.renderer.setSize(leftParams.renderWidth + rightParams.renderHeight, Math.min(leftParams.renderHeight, rightParams.renderHeight), false);
      });
    }
  }, {
    key: 'exitPresent',
...
FourByFourRectGeometry.prototype = Object.create(THREE.BufferGeometry.prototype);
...
function MergedBufferGeometry(geometries) {
  ...
  for (var _key in attributeDefs) {
    ...
    this.addAttribute(_key, new THREE.BufferAttribute(initialAttributes, attributeDefs[_key].itemSize));
  }

  var initialIndices = new Uint32Array(indexCount);
  this.setIndex(new THREE.BufferAttribute(initialIndices, 1));
...
MergedBufferGeometry.prototype = Object.create(THREE.BufferGeometry.prototype);
...
function BitmapFontGeometry(fontObject, text, fontHeight) {
  ...
  if (vAlign === BASELINE) {
    ...
    var md = font.MaxDescent;
    var fh = font.FontHeight;
    var maxFontHeight = ma + md;
    var maxTextHeight = (fh * (dim.numLines - 1) + maxFontHeight) * yScale;
    curPos[1] = (maxTextHeight - fontHeight) * 0.5 - md * yScale;
  } else if (vAlign === TOP) {
...
var InputControls = function () {
  function InputControls() {
    classCallCheck(this, InputControls);

    this.eventDispatcher = new THREE.EventDispatcher();
    this._inputSources = [];
  }

  createClass(InputControls, [{
    key: 'addInputSource',
    value: function addInputSource(inputSource) {
      inputSource.eventDispatcher.addEventListener(inputSource.inputEventType, this._onInputEvent.bind(this));
    ...
    value: function frame(targetElement) {
      for (var i = 0; i < this._inputSources.length; i++) {
        this._inputSources[i].frame && this._inputSources[i].frame(targetElement);
      }
    }
  }]);
...
var GamepadInputs = function () {
  function GamepadInputs() {
    classCallCheck(this, GamepadInputs);
...
var KeyboardInputs = function () {
  function KeyboardInputs() {
    classCallCheck(this, KeyboardInputs);
...
var MouseInputs = function () {
  function MouseInputs() {
    classCallCheck(this, MouseInputs);
...
var TouchInputs = function () {
  function TouchInputs() {
    classCallCheck(this, TouchInputs);
...
function GuiSys(root) {
  ...
  this.lastHit = null;
  this.lastHitType = GuiSysHitType.GAZE;
  this._lastHitCache = [];
  ...
  this._inputControls = new InputControls();
  this._inputControls.addInputSource(new GamepadInputs());
  this._inputControls.addInputSource(new KeyboardInputs());
  this._inputControls.addInputSource(new MouseInputs());
  this._inputControls.addInputSource(new TouchInputs());
...
GuiSys.prototype = Object.assign(Object.create(Object.prototype), {
  ...
  updateLastHit: function updateLastHit(hit, hitType) {
    ...
    if (this.lastHit !== currentHit || this.lastHitType !== hitType) {
      this.eventDispatcher.dispatchEvent(new GuiSysEvent(GuiSysEventType.HIT_CHANGED, { lastHit: this.lastHit, currentHit: currentHit,
    ...
    for (var id in this._lastHitCache) {
      if (!hitCache[id]) {
        this.eventDispatcher.dispatchEvent(new UIViewEvent(this._lastHitCache[id], UIViewEventType.FOCUS_LOST, { target: this.lastHit, hitType: this.lastHitType }));
      }
    }

    for (var _id in hitCache) {
      if (!this._lastHitCache[_id]) {
        this.eventDispatcher.dispatchEvent(new UIViewEvent(hitCache[_id], UIViewEventType.FOCUS_GAINED, { target: this.lastHit, hitType: this.lastHitType }));
      }
    }
  ...
  updateCursor: function updateCursor(camera) {
    ...
    if (this.cursorMesh && this.cursorEnabled) {
      var vector = new THREE.Vector3(0, 0, -cursorZ);
    ...
    if (this.cursorMesh) {
      this.cursorMesh.visible = this.cursorEnabled && (!this.cursorAutoHide || this.lastHit !== null && !!this.lastHit.isInteractable) && this.lastHitType === GuiSysHitType.GAZE;
    }
  },
...
function UIView(guiSys, params) {
  ...
  this.backgroundMaterial = new THREE.MeshBasicMaterial({ color: DEFAULT_BACKGROUND_COLOR, side: THREE.DoubleSide });
  this.backgroundMaterial.transparent = true;
  this.backgroundMaterial.visible = false;
  this.backgroundMaterial.depthWrite = false;
  this.borderMaterial = new THREE.MeshBasicMaterial({ color: DEFAULT_BORDER_COLOR, side: THREE.DoubleSide });
  this.borderMaterial.transparent = true;
  this.borderMaterial.visible = false;
  this.borderMaterial.depthWrite = false;
  this.imageMaterial = new THREE.MeshBasicMaterial({ color: DEFAULT_IMAGE_COLOR, side: THREE.DoubleSide });
...
UIView.prototype = Object.assign(Object.create(THREE.Object3D.prototype), {
  ...
  updateGeometry: function () {
    ...
    return function () {
      ...
      if (this.frameDirty) {
        ...
        var scaleParams = this.scaleType.getScaleParams([this.frame[2], this.frame[3]], this.textureDim, this.inset, this.insetSize, this.borderWidth);
        this.geometry = new MergedBufferGeometry([new CSSBorderGeometry([this.frame[2], this.frame[3]], this.borderWidth, this.borderRadius, 0.01, -0.0005, BORDER_MAT_INDEX, BACKGROUND_MAT_INDEX), new FourByFourRectGeometry(scaleParams.dims, scaleParams.border, scaleParams.cropUV, IMAGE_MAT_INDEX)]);
  ...
  applyUpdates: function applyUpdates(opacity) {
    this.updateGeometry();
  ...
  setFrame: function setFrame(x, y, width, height, animator) {
    ...
    if (animator) {
      (function () {
        var self = _this;
        var startFrame = [_this.frame[0], _this.frame[1], _this.frame[2], _this.frame[3]];
        var startTime = Date.now();
        var animState = _this.frame[2] === 0 && _this.frame[3] === 0 && animator.create ? animator.create : animator.update;
        ...
        _this.guiSys.cancelFrameFunction(self.animatorHandle);
  ...
  setImage: function setImage(url, loaded, invisibleTillLoad) {
    ...
    if (url) {
      ...
      loader.load(url, function (texture) {
        _this2.imageMaterial.map = texture;
        _this2.imageMaterial.mapurl = url;

        _this2.updateOffsetRepeat();
        _this2.imageMaterial.visible = true;
        _this2.imageMaterial.needsUpdate = true;
        loaded && loaded(true, texture.image.naturalWidth, texture.image.naturalHeight);
      }, function (xhr) {}, function (xhr) {
        _this2.imageMaterial.map && _this2.imageMaterial.map.dispose();
        _this2.imageMaterial.map = undefined;
        loaded && loaded(false);
      });
  ...
  setBorderWidth: function setBorderWidth(width) {
    if (width != this.borderWidth) {
      ...
      this.borderMaterial.visible = width > 0;
      this.frameDirty = true;
      this.dirtyGeometry = true;
    }
  },
  ...
  raycast: function () {
    ...
    return function raycast(raycaster, intersects) {
      ...
      vTL.fromArray([-this.frame[2] / 2 - this.hitSlop[0], this.frame[3] / 2 + this.hitSlop[1], 0]);
      vTR.fromArray([this.frame[2] / 2 + this.hitSlop[2], this.frame[3] / 2 + this.hitSlop[1], 0]);
      vBL.fromArray([-this.frame[2] / 2 - this.hitSlop[0], -this.frame[3] / 2 - this.hitSlop[3], 0]);
      vBR.fromArray([this.frame[2] / 2 + this.hitSlop[2], -this.frame[3] / 2 - this.hitSlop[3], 0]);
...
var Overlay = function () {
  function Overlay() {
    ...
    setStyles(buttons, BUTTON_WRAPPER_STYLES);
    var vrButton = document.createElement('a');
    this.vrButton = vrButton;
    setStyles(vrButton, VR_BUTTON_STYLES);
    vrButton.appendChild(document.createTextNode('View in VR'));
    ...
    overlayNode.appendChild(buttons);
  ...
  createClass(Overlay, [{
    ...
    value: function enableVRButton() {
      ...
      this.vrButton.addEventListener('click', this.handleVRButton);
    }
  }, {
    key: 'disableVRButton',
    value: function disableVRButton() {
      ...
      this.vrButton.removeEventListener('click', this.handleVRButton);
    }
  }, {
    key: 'setVRButtonText',
    value: function setVRButtonText(text) {
      this.vrButton.childNodes[0].nodeValue = text;
    }
  }, {
...
var Player = function () {
  function Player() {
    ...
    if (!this._camera) {
      this._camera = new THREE.PerspectiveCamera(60, width / height, 1, 2000);
    }
    this._initialAngles = {
  ...
  createClass(Player, [{
    ...
    value: function frame() {
      ...
      if (!(this.vrDisplay && this.vrDisplay.isPresenting && isMobile)) {
        if (this._camera.rotation.y !== this._lastAngle) {
          this._lastAngle = this._camera.rotation.y;
...
exports.GuiSysHitType = GuiSysHitType;
